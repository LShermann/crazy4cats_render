class ReactionsController < ApplicationController
    def new_user_reaction
        ...
        (@type == "comment") ? reaction_comment = Reaction.find_by(user_id: @user, comment_id: @comment.id) : reaction_article = Reaction.find_by(user_id: @user.id, post_id: @post.id)
            if reaction_article || reaction_comment
                format.html { redirect_to post_path(@post), notice: 'You already reacted to this article' }
            else
